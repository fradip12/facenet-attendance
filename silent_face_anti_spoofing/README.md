...
| Path | Purpose |
|---|---|
| `challenge_test.py` | **Entry point.** Live webcam demo: MediaPipe blink+smile challenges (randomized order) combined with the TFLite anti-spoof models' real/fake score |
| `resources/anti_spoof_models_tflite/*.tflite` | The two MiniFASNet model variants, ~1.8M each — their combined score decides real vs. fake |
| `resources/anti_spoof_models/*.pth` | Original PyTorch weights the `.tflite` files were converted from (via `litert-torch`) — kept for provenance; nothing in this folder runs them directly anymore |
| `src/generate_patches.py` | `CropImage` — expands the face bbox by each model's scale factor, crops, resizes to that model's input size |
| `src/utility.py` | `parse_model_name`/`get_kernel` — derives input size + scale from a model filename like `2.7_80x80_MiniFASNetV2.tflite` |
...
| Path | Fungsi |
|---|---|
| `challenge_test.py` | **Entry point.** Demo webcam langsung: challenge kedip+senyum MediaPipe (urutan acak) digabung dengan skor asli/palsu dari model anti-spoof TFLite |
| `resources/anti_spoof_models_tflite/*.tflite` | Dua varian model MiniFASNet, ~1.8M masing-masing — skor gabungannya menentukan asli vs. palsu |
| `resources/anti_spoof_models/*.pth` | Bobot PyTorch asli yang menjadi sumber konversi ke `.tflite` (via `litert-torch`) — disimpan untuk keperluan provenance; tidak ada kode di folder ini yang menjalankannya langsung lagi |
| `src/generate_patches.py` | `CropImage` — memperluas bounding box wajah sesuai faktor skala tiap model, memotong (crop), lalu me-resize ke ukuran input model tersebut |
| `src/utility.py` | `parse_model_name`/`get_kernel` — menentukan ukuran input + skala dari nama file model seperti `2.7_80x80_MiniFASNetV2.tflite` |
